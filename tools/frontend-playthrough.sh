...
# F4 說明頁（docs/spec/012 §3）：按一次要出現、再按一次要回到原畫面，而且鍵不進遊戲
h0=\$(shot); tap F4; sleep 2; h1=\$(shot); import -window \"\$W\" \$OUT/help.png; tap F4; sleep 2; h2=\$(shot)
{ [ \"\$h1\" != \"\$h0\" ] && echo 'F4 開啟：畫面變了' || echo 'F4 開啟：畫面沒變（說明頁沒出現）'
  [ \"\$h2\" = \"\$h0\" ] && echo 'F4 關閉：回到原畫面' || echo 'F4 關閉：畫面與原本不同'; } > \$OUT/f1.txt
