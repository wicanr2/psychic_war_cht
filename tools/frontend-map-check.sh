#!/usr/bin/env bash
# F6 自動地圖的實跑驗收（docs/spec/015 §4 第 2、3 項）。
#
#   tools/frontend-map-check.sh [狀態檔] [走幾步]     # 預設 states/07-first-play.state、3 步
#
# 流程：Xvfb 裡從狀態檔啟動 → 往前走 N 步 → 按 F6 截圖 → 再按 F6 關掉 → F10 存檔。
# 比對（tools/map_check.py）：
#   目前格子（由存檔讀出的 X、Y）在畫面上是白色；走過的格子是灰色；沒走過的格子是黑色（反向對照）。
# 記憶體：開關 F6 前後的 lin:16966 起 52 bytes 相同（用 F10 存檔 ＋ probe 比）。
...
echo "開關 F6 前後的觀測變數：$mem" | tee -a "$ROOT/$OUT/result.txt"
[[ "$mem" == "相同" ]] || rc=1
exit $rc
