#!/usr/bin/env bash
# DOSBox-X 錄標題音樂（PC 喇叭路徑），給 issue #6 的音樂比對當參照。
#
#   tools/dosboxx-audio.sh [秒數]      # 預設 95 秒；產出 workplace/dosboxx-audio/title.wav
#
# 不按任何鍵：標題畫面會一直等，音樂照樣播（OPEN0 → OPEN1 → …，約 84 秒一輪）。
# 音效卡設成沒有（sbtype=none、oplmode=none），遊戲偵測不到 AdLib，走 .IBM 的 PC 喇叭路徑，
# 與 dosgolem 預設相同。
#
...
mkdir -p "$OUT"
rm -rf "$OUT"/capture "$OUT"/title.wav
...
echo "tools/dosboxx-audio.sh：跑 DOSBox-X 錄 $SECS 秒（1 個容器、2 核）" >&2
