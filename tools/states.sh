...
#
# 七段，每一段從上一段的狀態檔接著跑、送一組按鍵、在固定指令數存狀態（-save-state）
# 並存一格畫面。四個 M0 檢查點是：
#
#   01-title        標題畫面，等按鍵
...
for row in "${STAGES[@]}"; do
  ...
  [[ -s "$OUT/$name.state" && -s "$OUT/$name.frame" ]] || die "$name 沒有產出狀態檔或畫面（見 $OUT/$name.log）"
...
if (( CHECK )); then
  ...
  if diff <(cut -f1-3 "$EXPECTED") "$manifest"; then
    echo "決定性檢查：七段畫面雜湊與 tools/states.expected 相同" >&2
  else
