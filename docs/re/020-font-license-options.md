...
狀態：比較紀錄（2026-09-17）。**使用者定案 2026-09-18：採方案 A，發行也用倚天字形**（見 §4）。對應 issue #21、#36。
...
**使用者定案 2026-09-18：方案 A，發行版也用倚天字形。** 理由是字形觀感要貼近當年代理版；授權風險由使用者承擔。

- 字型子集（`font/cjk24.golemfnt`、`cjk16.golemfnt`）只含譯文用到的字，隨發行物散布。
- README（#36）要寫明字模來源是倚天中文系統 3.53 的點陣字、只取子集，並附收到權利人異議時的聯絡與下架方式。
- 方案 B–E 留著當備案：切換字型只要重跑 `tools/font/bake.sh` 並改 `docs/spec/009` §2 的字模偏移，疊字層與驗收工具不用改。

原本的建議（公開前改用 D，另留 E）沒有採用。
