...
- **文本檔的原文欄位進版控（使用者定案 2026-09-19，不重問）。** `text/*.json` 的 `original`
  是原版的遊戲字串，轉譯層要靠它算長度與對位。原本的規則是「公開 repo 之前要改成從玩家自備的
  執行檔重抽」，使用者明示解除：**repo 公開，原文欄位照留**。
  `tools/text_extract.py build <原版目錄>` 仍然能從原版重建 `text/`，那是驗證工具不是發行前提。
...
- **發行包直接帶 `text/` 的原文欄位（使用者定案 2026-09-19，`docs/spec/021` §2，不重問）**：轉譯層要靠原文對位，
  而沒有 `PW.EXE` 遊戲本來就跑不起來。硬規則「含原文的文本檔不得進公開 repo」管的是 repo 本身，仍然成立。
- **推廣片的配樂要用 `workplace/dosboxx-audio/title-adlib.wav`**（DOSBox-X 跑原版錄下來的），
  **不可以用 `workplace/audio/golem-opl-title.wav`**（我們自己寫的合成器算的）——`rulebook/93` 鐵則 1。
  影片要對外公開之前，先提醒使用者原版音樂的著作權面向（同規則的但書）。
